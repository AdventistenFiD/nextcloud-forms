...
- Restrictions:
    ...
    - `extraSettings.requireEmailVerification` requires `extraSettings.validationType = email` and `extraSettings.confirmationRecipient = true`.
...
- Restrictions: The array of answers has the following structure:
    ...
    - If the same question also sets `requireEmailVerification = true`, the submission stays unverified until the recipient opens the verification link, and the confirmation email is only sent after verification succeeds.
